...
public class ReviewDAO_imple implements ReviewDAO {
   ...
   // 리뷰 작성 가능한 목록보기
   @Override
   public List<ReviewVO> myreviewUpList(String m_id) throws SQLException {
      ...
      try {
         ...
         while(rs.next()) {
            ...
            rvo.setPvo(pvo);
             ...
             OptionVO opvo = new OptionVO();        
             String getMl = opvo.getOp_ml();
      //       opvo.getMl(rs.getString("op_ml"));
                opvo.setOp_ml(rs.getString("op_ml"));
                System.out.println("opvo.getOp_ml()"+opvo.getOp_ml());
                rvo.setOpvo(opvo);
   ...
   // 구매후기를 작성하였는지 알아오기
   @Override
   public boolean reviewCheck(String m_id, int p_num) throws SQLException {
      ...
      try {
           ...
           String sql = " select r_num "
                 + " from tbl_review "
                 + " where fk_m_id =? and fk_p_num = ? "
